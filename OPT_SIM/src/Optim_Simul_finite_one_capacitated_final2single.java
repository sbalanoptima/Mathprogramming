...
public class Optim_Simul_finite_one_capacitated_final2single {
			...
			for(int t=0; t<timeLimit; t++) {
				...
				for(int l=1; l<limitD[t]; l++) {
					for(int q=1; q<limitD[t]; q++) {
						cplex.addLe(cplex.sum(y[l][t],y[q][t]),cplex.sum(cplex.prod(2, E[l][q][t]),1) ,"new1");
						cplex.addGe(cplex.sum(y[l][t],cplex.prod(-1, y[q][t])), cplex.sum(cplex.prod(1, E[l][q][t]),-1),"new2");
						cplex.addGe(cplex.sum(y[q][t],cplex.prod(-1, y[l][t])), cplex.sum(cplex.prod(1, E[l][q][t]),-1),"new3");
						cplex.addGe(cplex.sum(y[l][t],y[q][t]),cplex.prod(1, E[l][q][t]),"new4");
					}
