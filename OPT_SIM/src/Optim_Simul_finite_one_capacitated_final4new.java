...
public class Optim_Simul_finite_one_capacitated_final4new {
	...
	public static void main(String[] args) {		
		...
		double[] meanConstant 				= {9,9,9,9,9,9,9,9,9,9};
		...
		try { 
			...
			for(int t=1; t<timeLimit; t++) {
				for(int l=0; l<limitD[t]; l++) {
				    ...
				    //if(q==l) {
				    	//fullexpr = cplex.sum(fullexpr, expr1, expr3);
				    	//continue;
				   // }
				    IloNumExpr expr4 = cplex.prod(pplushprime, cplex.prod(p1l, E[l][q][t]));
			...
			cplex.addEq(IN[0], cplex.sum(S1[1], -9),"con6"); 
			cplex.addEq(X[0], 0,"con5");
			
			// S[t] = delta * sum of strips equation
			for(int t=0; t<timeLimit; t++) {
			// consecutive 1's on yg and sumyg = S1
				for(int g=1; g<limitD[t]; g++) {
					cplex.addGe(y[g-1][t], y[g][t],"con2");
				}
				cplex.addEq(cplex.prod(delta,sumYg[t]), S1[t],"con7");
				cplex.addEq(cplex.prod(delta,sumY1g[t]), S1[t],"con8");
				if(t>1) {
					// S1 can be maximum increased with its capacity
					cplex.addLe(S1[t], cplex.sum(S1[t-1], capacity[t]),"con9");
				}
			}

			// indicator function for relaxing S
			for(int t=0; t<timeLimit; t++) {
				cplex.addLe(cplex.sum(1, cplex.prod(-1,z[t])), X[t],"con10");
				cplex.addGe(cplex.prod(M, cplex.sum(1, cplex.prod(-1,  z[t]))), X[t],"con11");
				if(t>0) {
					cplex.addGe(cplex.sum(S1[t], cplex.prod(M, u[t])), cplex.sum(S1[t-1], -mean[t-1]),"con12");
					cplex.addLe(cplex.sum(0,S1[t]), cplex.sum(S1[t-1], cplex.sum(-mean[t-1],cplex.sum(M, cplex.prod(-M, u[t])))),"con13");					
				}	
				// Linearizing z[t] and u[t] using v[t]
				cplex.addLe(v[t], z[t]);
				cplex.addLe(v[t], u[t]);
			...
			cplex.exportModel ("optim4new.lp");		
