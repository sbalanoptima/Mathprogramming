...
public class Optim_Simul_finite_one_capacitated_final2 {
		...
		try { 
			...
			for(int t = 0; t<timeLimit; t++) {
				...
				for(int g=0; g<limitD[t]; g++) {
					y[g][t] = cplex.boolVar("y"+g+t);
				}
				for(int q=0; q<limitD[t]; q++) {
					y1[q][t] = cplex.boolVar("y1"+q+t);
				}
				for(int g=0; g<limitD[t]; g++) {
					for(int q=0; q<limitD[t]; q++) {
						E[g][q][t] = cplex.boolVar("E"+g+q+t);
					}
				}
			}
			...
			for(int t=1; t<timeLimit; t++) {
				for(int l=0; l<limitD[t]; l++) {
					...
					IloNumExpr expr2 = cplex.prod(pplushprime, cplex.prod(cplex.sum(lDp1l, cplex.prod(-p1l, S1[t])), cplex.sum(1, cplex.prod(-1, y[l][t]))));	
				...
				if(t>0) {
					...
					// relaxing S
					cplex.addGe(cplex.sum(IN[t-1], cplex.sum(X[t], X[t-1])), S1[t], "con.31");
					cplex.addLe(cplex.sum(IN[t-1], cplex.sum(X[t], X[t-1])), cplex.sum(S1[t],cplex.prod(M, v[t])), "con.32");
				}
			...
			cplex.addEq(IN[0], cplex.sum(S1[1], -9),"con.5"); 
			cplex.addEq(X[0], 0,"con.6");
			...
			for(int t=0; t<timeLimit; t++) {
				...
				cplex.addEq(cplex.prod(delta,sumYg[t]), S1[t],"con.7");
				if(t>1) {
					// S1 can be maximum increased with its capacity
					cplex.addLe(S1[t], cplex.sum(S1[t-1], capacity[t]),"con.8");
				}
			}

			// indicator function for relaxing S
			for(int t=0; t<timeLimit; t++) {
				cplex.addLe(cplex.sum(1, cplex.prod(-1,z[t])), X[t],"con9.1");
				cplex.addGe(cplex.prod(M, cplex.sum(1, cplex.prod(-1,  z[t]))), X[t],"con9.2");
				if(t>0) {
					cplex.addGe(cplex.sum(S1[t], cplex.prod(M, u[t])), cplex.sum(S1[t-1], -mean[t-1]),"con10.1");
					cplex.addLe(cplex.sum(0,S1[t]), cplex.sum(S1[t-1], cplex.sum(-mean[t-1],cplex.sum(M, cplex.prod(-M, u[t])))),"con10.2");					
				}	
				cplex.addLe(v[t], z[t],"con11.1");
				cplex.addLe(v[t], u[t],"con11.2");
				cplex.addGe(v[t], cplex.sum(z[t], cplex.sum(u[t], -1)),"con11.3");
				for(int l=1; l<limitD[t]; l++) {
					for(int q=1; q<limitD[t]; q++) {
			...
			cplex.exportModel ("originalIP.lp");		

			long startTime = System.currentTimeMillis();

			// parameters for CPLEX
			cplex.setParam(IloCplex.Param.TimeLimit, 10800); // time limit not required // Given 3 hours maximum run time
			cplex.setParam(IloCplex.Param.MIP.Cuts.BQP, -1); // Removed Boolean quadratic program cuts from solver algorithms
			cplex.setParam(IloCplex.Param.Preprocessing.QCPDuals,0); //Cplex does not calculate dual values for the QCP
			cplex.setParam(IloCplex.Param.Preprocessing.CoeffReduce, 0); //cplex turns off coefficient reduction during preprocessing.
			cplex.setParam(	IloCplex.Param.MIP.Cuts.Covers, -1); //Do not generate cover cuts
			cplex.setParam(	IloCplex.Param.MIP.Cuts.Cliques, -1); //Do not generate clique cuts
			cplex.setParam(IloCplex.Param.MIP.Cuts.Disjunctive, -1); //Do not generate disjunctive cuts
			cplex.setParam(IloCplex.Param.MIP.Limits.EachCutLimit, 0); //No cuts
			cplex.setParam(	IloCplex.Param.MIP.Cuts.FlowCovers, -1); //Do not generate flow cover cuts
			cplex.setParam(	IloCplex.Param.MIP.Cuts.PathCut, -1); //Do not generate flow path cuts
			cplex.setParam(	IloCplex.Param.MIP.Cuts.Gomory, -1); //Do not generate gomory fractional cuts
			cplex.setParam(IloCplex.Param.MIP.Cuts.GUBCovers, -1); ////Do not generate GUB cuts
			cplex.setParam(	IloCplex.Param.MIP.Cuts.Implied, -1); //Do not generate Implied bound cuts
			cplex.setParam(	IloCplex.Param.MIP.Cuts.LocalImplied, -1); //Do not generate local implied bounds cuts
			cplex.setParam(	IloCplex.Param.MIP.Cuts.LiftProj, -1); //Do not generate Lift and project cuts for MIQP
			cplex.setParam(	IloCplex.Param.MIP.Cuts.MCFCut, -1); //Do not generate multi commodity flow cuts
