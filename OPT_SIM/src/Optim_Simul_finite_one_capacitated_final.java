...
public class Optim_Simul_finite_one_capacitated_final {
		...
		try { 
			...
			for(int t=0; t<timeLimit; t++) {
				...
				if(t>0) {
					...
					cplex.addLe(cplex.sum(1,S1[t]), cplex.sum(S1[t-1], cplex.sum(-mean[t-1],cplex.sum(M, cplex.prod(-M, u[t])))),"con10.2");					
				}	
			...
			cplex.setParam(IloCplex.Param.TimeLimit, 10800); // time limit not required
	...
	public static void writeToFile(ArrayList<OptResult> optResults, ArrayList<SimResult> simResults) {
		...
		File file = new File(destination + "/opt_and_sim20.txt");
